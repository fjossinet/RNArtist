...
class InteractionKw(var parent:DSLElement, script: Script, indentLevel:Int): OptionalDSLKeyword(script, " interaction ", indentLevel) {
    ...
    init {
        this.children.add(1, DSLParameter(script, key = StringWithoutQuotes(script, "orientation") , operator = Operator(script, " = "), StringWithQuotes(script, value = "cis", editable = true),  this.indentLevel + 1))
        this.children.add(1, DSLParameter(script, key = StringWithoutQuotes(script, "pos2") , operator = Operator(script, " = "), StringWithoutQuotes(script, editable = true),  this.indentLevel + 1))
        this.children.add(1, DSLParameter(script, key = StringWithoutQuotes(script, "edge2") , operator = Operator(script, " = "), StringWithQuotes(script, value = "wc", editable = true),  this.indentLevel + 1))
        this.children.add(1, DSLParameter(script, key = StringWithoutQuotes(script, "pos1") , operator = Operator(script, " = "), StringWithoutQuotes(script, editable = true),  this.indentLevel + 1))
