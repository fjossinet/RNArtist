...
class BracketNotationKw(val parent:SecondaryStructureInputKw?, script: Script, indentLevel:Int, inFinalScript:Boolean = false): OptionalDSLKeyword(script,  " bn", indentLevel, inFinalScript) {
    ...
    override fun addToFinalScript(add:Boolean) {
        ...
        if (add) {
            ...
            val bnParameter = DSLParameter(script, StringWithoutQuotes(script,"value"), Operator(script,"="), StringValueWithQuotes(script,"click me to set your structure", true), this.indentLevel+1)
            this.children.add(1, bnParameter)
            this.children.add(1, SequenceBnParameter(script, bnParameter, this.indentLevel+1))
