...
import io.github.fjossinet.rnartist.io.parseScript
...
class ScriptEditor(val mediator: Mediator) {
    ...
    private fun createScene(stage: Stage) {
        ...
        loadFile.setOnAction(EventHandler<ActionEvent?> {
            val fileChooser = FileChooser()
            val file = fileChooser.showOpenDialog(stage)
            file?.let {
                //convert script to editor model
               val root = parseScript(FileReader(file), this)
                when (root) {
                    is RNArtistKw -> {
                        scriptRoot = root
                        initScript()
        ...
        loadGist.setOnAction(EventHandler<ActionEvent?> {
            ...
            gistInput.editor.text = "1fd35e371bd8726c5b8e63ac04b62d83"
            ...
            if (gistID.isPresent && !gistID.isEmpty) {
                ...
                val scriptContent = "${match?.groupValues?.get(1)}".
                                ...
                                replace("\\","")
                //convert script to editor model
                val root = parseScript(StringReader(scriptContent), this)
                when (root) {
                    is RNArtistKw -> {
                        scriptRoot = root
                        initScript()
        ...
        run.onAction = EventHandler {
            Platform.runLater(Runnable() {
                try {
                    val result = engine.eval(getScriptAsText())
                    when (result) {
                        is List<*> -> {
                            (result as? List<SecondaryStructureDrawing>)?.forEach {
                                //We check if a 2D with the same uniq id is not already loaded (meaning a previous run of the same script)
                                //If we find one, we remove the previous one otherwise we will have a 2D for each run
                                var found: DrawingLoadedFromScriptEditor? = null
                                mediator.drawingsLoaded.forEach { drawingLoaded ->
                                    if (drawingLoaded is DrawingLoadedFromScriptEditor && drawingLoaded.id.equals(
                                            scriptRoot!!.id
                                        )
                                    ) {
                                        found = drawingLoaded
                                ...
                                found?.let {
                                    mediator.drawingsLoaded.remove(it)
                                }
        ...
        leftToolbar.children.addAll(increaseLineSpacing, decreaseLineSpacing, increaseTab, decreaseTab, spacer, Label("Bg"), bgColor, Label("Kw"), kwColor, Label("{ }"), bracesColor, Label("Key"), keyParamColor, Label("Op"), operatorParamColor, Label("Val"), valueParamColor)
