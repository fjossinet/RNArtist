...
class ResidueShapesColorPicker(mediator: Mediator) : RNArtistColorPicker(mediator) {

    override var behaviors = mapOf(
        Pair("Any") { e: DrawingElement -> e is ResidueDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain().any { mediator.canvas2D.getSelection().contains(it) } else true},
        Pair("Adenines") { e: DrawingElement -> e is AShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain().any { mediator.canvas2D.getSelection().contains(it) } else true},
        Pair("Uracils") {e:DrawingElement -> e is UShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain().any { mediator.canvas2D.getSelection().contains(it) } else true},
        Pair("Guanines") {e:DrawingElement -> e is GShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain().any { mediator.canvas2D.getSelection().contains(it) } else true},
        Pair("Cytosines") {e:DrawingElement -> e is CShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain().any { mediator.canvas2D.getSelection().contains(it) } else true},
        Pair("Purines") {e:DrawingElement -> (e is AShapeDrawing || e is GShapeDrawing) && if (this.restrictedToSelection) e.pathToStructuralDomain().any { mediator.canvas2D.getSelection().contains(it) } else true},
        Pair("Pyrimidines") {e:DrawingElement -> (e is UShapeDrawing || e is CShapeDrawing) && if (this.restrictedToSelection) e.pathToStructuralDomain().any { mediator.canvas2D.getSelection().contains(it) } else true},
        Pair("Unknowns") {e:DrawingElement -> e is XShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain().any { mediator.canvas2D.getSelection().contains(it) } else true}
    )
    ...
    init {
        ...
        randomColors.onMouseReleased = EventHandler {
            ...
            mediator.drawingDisplayed.get()?.let { drawing ->
                ...
                if (this.targetsComboBox.value in listOf("Any", "Adenines", "Purines")) {
                    val colorA = getHTMLColorString(randomColor())
                    t.setConfigurationFor(selection = { e: DrawingElement ->
                        e is AShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain()
                            .any { mediator.canvas2D.getSelection().contains(it) } else true
                    }, ThemeParameter.color) {
                        colorA
                    }
                }
                if (this.targetsComboBox.value in listOf("Any", "Uracils", "Pyrimidines")) {
                    val colorU = getHTMLColorString(randomColor())
                    t.setConfigurationFor(selection = { e: DrawingElement ->
                        e is UShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain()
                            .any { mediator.canvas2D.getSelection().contains(it) } else true
                    }, ThemeParameter.color) {
                        colorU
                    }
                ...
                if (this.targetsComboBox.value in listOf("Any", "Guanines", "Purines")) {
                    val colorG = getHTMLColorString(randomColor())
                    t.setConfigurationFor(selection = { e: DrawingElement ->
                        e is GShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain()
                            .any { mediator.canvas2D.getSelection().contains(it) } else true
                    }, ThemeParameter.color) {
                        colorG
                    }
                ...
                if (this.targetsComboBox.value in listOf("Any", "Cytosines", "Pyrimidines")) {
                    val colorC = getHTMLColorString(randomColor())
                    t.setConfigurationFor(selection = { e: DrawingElement ->
                        e is CShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain()
                            .any { mediator.canvas2D.getSelection().contains(it) } else true
                    }, ThemeParameter.color) {
                        colorC
                    }
                ...
                if (this.targetsComboBox.value in listOf("Any", "Unknowns")) {
                    val colorX = getHTMLColorString(randomColor())
                    t.setConfigurationFor(selection = { e: DrawingElement ->
                        e is XShapeDrawing && if (this.restrictedToSelection) e.pathToStructuralDomain()
                            .any { mediator.canvas2D.getSelection().contains(it) } else true
                    }, ThemeParameter.color) {
                        colorX
                    }
